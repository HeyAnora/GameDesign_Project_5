...
namespace UnfinityGames.U2DEX
{
	...
	public static class AdvancedSnapping
	{
		...
		//A private variable that we'll use to cache our previously-selected Transform, so we can decide when our Selection has changed.
		private static Transform previouslySelectedTransform = null;
		...
		{
			//If we're in play mode, we don't want to update Snapping.
			//Return early.
			if(EditorApplication.isPlaying)
				return;

			//If we don't have anything selected, we don't need to update Snapping.
			//Return early.
			if(Selection.transforms.Length <= 0)
				return;

			//If our activeTransform and our previouslySelectedTransform don't match, our selected object has changed.
			if(Selection.activeTransform != previouslySelectedTransform)
			{
				//Update our previouslySelectedTransform.
				previouslySelectedTransform = Selection.activeTransform;

				//Set our ApplicableObjectSelected to false, as we haven't had enough time for the Inspector to figure out if
				//the current object is selected.
				GlobalSnappingData.ApplicableObjectSelected = false;

				//Return early, as we don't want to process snapping this frame (as our selection just changed).
				return;
			}
