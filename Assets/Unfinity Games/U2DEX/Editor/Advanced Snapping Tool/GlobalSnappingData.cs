...
namespace UnfinityGames.U2DEX
{
	...
	public static class GlobalSnappingData
	{
		...
		public static readonly string[] SupportedClasses = new string[3] { "tk2dBaseSprite", "OTSprite",
																				"SpriteRenderer"};
		/// <summary>
		/// A list containing the string name of all of our class types that should be disabled (i.e. excluded from having the U2DEX Inspector).
		/// </summary>
		public static List<string> DisabledClasses
		{
			get
			{
				//if we have some data loaded, then go ahead and set this to the loaded data
				if(PrivateSnapData != null)
				{
					return PrivateSnapData.disabledClasses;
				}
				else //otherwise, if we don't have any data loaded, set it to the default
				{
			...
			set
			{
				...
				if(PrivateSnapData != null)
				{
					PrivateSnapData.disabledClasses = value;
				}
			}
		}
