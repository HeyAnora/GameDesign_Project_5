	...
	public class AdvancedSnapping_HelpUI : EditorWindow
	{
		...
		public void OnGUI()
		{
			...
			EditorGUILayout.BeginHorizontal();
			{
				...
				GUILayout.Label(new GUIContent(addClass, "A diagram representing an added class."), GUILayout.MinWidth(474));
				GUILayout.FlexibleSpace();
			}
			...
			GUILayout.Label("• Toggle Class", headingStyle);
			GUILayout.Label("'Toggle Class' can be used to selectively disable (and then re-enable) classes/types." +
							"  When a class is disabled, the U2DEX Transform Inspector will not be applied to it." +
							"  This is especially useful for disabling the officially-supported, built-in types, as they cannot be deleted." +
							"\n\nAs an added bonus, it can be used to disable classes without deleting them, in case you need to re-enable them later.",
							textStyle);

			GUILayout.Space(15);

			GUILayout.Label("• Remove Class", headingStyle);
			GUILayout.Label("'Remove Class' also does exactly what it advertises." +
