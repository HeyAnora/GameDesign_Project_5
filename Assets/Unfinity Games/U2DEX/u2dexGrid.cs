...
namespace UnfinityGames.U2DEX
{
	...
	public class u2dexGrid : MonoBehaviour
	{
		...
		{
			//Note: the Mathf.Round function still rounds to a whole number.  To get around this we multiply by 100 first,
			//and then divide the resulting number by 100 to make sure that we maintain the ability to round to fractional
			//numbers.
			return SnapTo * Mathf.Round((input / SnapTo) * 100f) / 100f;
		}
